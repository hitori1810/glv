...
(function(app) {
    ...
    app.view.Field = app.view.Component.extend({
        ...
        _loadTemplate: function() {
            ...
            var viewName = this.options.def.view || this.options.viewName ||
                (this.view.meta && this.view.meta.type ? this.view.meta.type : this.view.name);

            var actionName = viewName != this.view.action ? this.view.action : viewName;

            while (viewName) {

                if (app.acl.hasAccessToModel(actionName, this.model, this.name)) break;
            ...
            if (viewName) {
                ...
                var moduleName = this.context.get('module');

                fallbackFieldTemplate = this.view.fallbackFieldTemplate || "detail";
        ...
        _render: function() {
            ...
            var fieldHtml = this.template(this) || '';
            if (app.config.env === "test") {
                this.$el.html(fieldHtml).addClass('fld_' + this.name);
            } else {
        ...
        setDisabled: function(disable) {
            ...
            this.getFieldElement().removeClass(this.def.css_class);
            var css_class = this.def.css_class.split(' ');
            if(_.isUndefined(disable) || disable) {
                this._previousViewName = this.options.viewName;
                this.options.viewName = 'detail';
                css_class.push('disabled');
            } else if(this._previousViewName) {
                this.options.viewName = this._previousViewName;
                css_class = _.without(css_class, 'disabled');
            } else {
                delete this.options.viewName;
                css_class = _.without(css_class, 'disabled');
            }
            //"disabled" attched css will be applied while rendering time
