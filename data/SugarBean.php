...
//    	require_once('data/Link.php');
    ...
    */
    ...
    protected function addParentRecordsToResave($lname) {
        ...
        if (!empty($this->field_defs['parent_type']) && $this->field_defs['parent_type']['type'] == 'parent_type'
            && !empty($this->field_defs['parent_id']) && $this->field_defs['parent_id']['type'] == 'id'
            && !empty($this->parent_id) && $this->parent_id != $this->fetched_row['parent_id']
    ...
    */
    ...
    function fixUpFormatting()
    {
        ...
        foreach($this->field_defs as $field=>$def)
            ...
            if ( (isset($def['source'])&&$def['source']=='non-db') || $field == 'deleted' || $field == 'base_rate') {
                continue;
            }
    ...
    */
    function fill_in_additional_parent_fields()
    {
        ...
        if(!empty($this->parent_type)) {
            ...
            } elseif (isset($this->parent_document_name) && $this->parent_document_name != '') {
...
//					$this->format_field($value);
                ...
                //Fix bug load not need to load - By Lap Nguyen
                if($field == 'assigned_user_name' && empty($return_array['ASSIGNED_USER_NAME'])){
    ...
    * @return STRING
    */
    function getOwnerWhere($user_id, $tableAlias = '')
    {
        if(isset($this->field_defs['assigned_user_id']))
        {
            if (!empty($tableAlias)) {
                return " $tableAlias.assigned_user_id ='$user_id' ";
            } else {
        ...
        {
            if (!empty($tableAlias)) {
                return " $tableAlias.created_by ='$user_id' ";
            } else {
