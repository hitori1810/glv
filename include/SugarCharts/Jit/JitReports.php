    ...
    class JitReports extends Jit {
        ...
        */
        ...
        function xmlDataReportChart()
        {
            ...
            $first = true;
            //Custom by Lap Nguyen - Show % in Report Chart
            $total_value = 0;
            foreach ($this->data_set as $key => $dataset){
                $total_ = $this->calculateReportGroupTotal($dataset);
                if($total_ < 0)
                    $total_ = 0;
                $total_value +=  $total_;  
            }        
            foreach ($this->data_set as $key => $dataset){
                ...
                if($this->chart_properties['type'] == 'pie chart'){
                    $total__ = $total;
                    if($total < 0)
                        $total__ = 0;    
                    $data .= $this->tabValue('label', ''.round($total__/$total_value*100,1).'%', 3);
                }
