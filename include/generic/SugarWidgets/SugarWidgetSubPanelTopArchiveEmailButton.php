...
class SugarWidgetSubPanelTopArchiveEmailButton extends SugarWidgetSubPanelTopButton
{
	function display($defines)
	{
		...
		if(isset($defines['focus']->email1))
		{
            ...
            // Bug63165: missing Name in front of <Email Address>
            if (!empty($defines['focus']->full_name)) {
                $additionalFormFields['to_email_addrs'] = $defines['focus']->full_name ." &lt;".$additionalFormFields['to_email_addrs'] ."&gt;;";
            }
		}
		if(ACLController::moduleSupportsACL($defines['module'])  && !ACLController::checkAccess($defines['module'], 'edit', true)){
