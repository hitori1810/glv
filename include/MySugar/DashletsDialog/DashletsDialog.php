...
class DashletsDialog {
    ...
    function getDashlets($category='') {
        ...
        foreach($dashletsFiles as $className => $files) {
            // Added by Hieu Nguyen on 2017-08-09 to hide dashlets of disabled modules
            require('custom/include/DisabledModules.php');
            $dashletPath = $files['file'];
            $moduleName = '';

            if(strpos($dashletPath, 'custom') === 0) {
                $dashletPath = str_replace('custom/modules/', '', $dashletPath);
                $moduleName = substr($dashletPath, 0, strpos($dashletPath, '/'));
            }
            else {
                $dashletPath = str_replace('modules/', '', $dashletPath);
                $moduleName = substr($dashletPath, 0, strpos($dashletPath, '/'));
            }

            if(in_array($moduleName, $disabledModules)) {
                // Don't return this dashlet if it's from the disabled module
                continue;
            }
            // End Hieu Nguyen
...
//                $where = "AND saved_reports.team_set_id='1'";
            ...
            case 'myTeams':
                ...
                if (!is_admin($GLOBALS['current_user'])) {
                    $where = " AND saved_reports.team_set_id IN
