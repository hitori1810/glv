...
class SugarSpot
{
    ...
    protected function _performSearch($query, $modules, $offset = -1, $limit = 20, $options = array())
    {
        ...
        foreach($modules as $moduleName)
        {
            ...
            $searchForm = $this->getSearchFormClass($seed, $moduleName);
    ...
    /**
     * Get class for search form
     * 
     * @param string $module moduleDir
     * @param bean $seedBean seed bean to use
     * @return SearchForm
     */
    private function getSearchFormClass($module, $seedBean)
    {
        SugarAutoLoader::requireWithCustom('include/SearchForm/SearchForm2.php');
        $class = SugarAutoLoader::customClass('SearchForm');
        return new $class($module, $seedBean);
    }
}
