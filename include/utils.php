...
function get_sugar_config_defaults() {
    ...
    */
    ...
    $sugar_config_defaults = array (
        ...
        'default_currency_significant_digits' => return_session_value_or_default('default_currency_significant_digits', 0),
...
*/
function get_user_array($add_blank=true, $status="", $user_id='', $use_real_name=false, $user_name_filter='', $portal_filter=' AND portal_only=0 ', $from_cache = true) {
    global $locale, $sugar_config, $current_user;

    //add by Trung Nguyen for not show portal users
    $custom_query = " AND (l1.for_portal_only = 0) AND (l1.portal_user <> 1)";

    if(!$current_user->isAdmin())
        $custom_query .= " AND (teams.id = '{$current_user->team_id}')";

    if(!empty($status))
        $custom_query .= " AND (l1.status='$status')";

    if($from_cache) {
        $key_name = $add_blank. $status . $user_id . $use_real_name . $user_name_filter . $portal_filter . $custom_query;
        $user_array = get_register_value('user_array', $key_name);
    }

    if (!empty($user_name_filter)) {
        $user_name_filter = $db->quote($user_name_filter);
        $custom_query .= " AND (l1.user_name LIKE '$user_name_filter%') ";
    }
    if (!empty($user_id)) {
        $custom_query .= " OR l1.id='{$user_id}'";
    }
    //end
    if(empty($user_array)) {
        $db = DBManagerFactory::getInstance();
        ...
        IFNULL(l1.id, '') id,
        IFNULL(l1.first_name, '') first_name,
        IFNULL(l1.last_name, '') last_name,
        ...
        $order_by_string =' ORDER BY user_name ASC ';
        if (empty($locale)){
            $locale = Localization::getObject();
        }

        //get the fields used for name formatting, to be used in order by
        if(!empty($current_user)){
            $formatFields = $locale->getNameFormatFields($current_user);

            //create the order by string based on position of first and last name in format string
            if(!empty($formatFields)){
                $order_by_string = implode(', ',$formatFields).' ASC';
            }
        }
...
function getUserArrayFromFullName($args, $hide_portal_users = false) {
    ...
    if ( $hide_portal_users ) {
        $query .= " portal_only=0 AND portal_user=0 AND for_portal_only=0 AND ";
    }
    $query .= $inClause;
...
function get_select_options_with_id_separate_key ($label_list, $key_list, $selected_key, $massupdate=false) {
    ...
    if (is_array($selected_key)) {
        $flipped_selected_key = array_flip($selected_key);
    }
    ...
    foreach ($key_list as $option_key=>$option_value) {
        ...
        if (
            ...
            || (is_array($selected_key) &&  isset($flipped_selected_key[$option_key]))
            ) {
        ...
        foreach ($gimp as $domKey => $domVal) {
            if (isset($dom[$domKey])) {
                if (is_array($dom[$domKey]) && $dom[$domKey] !== $gimp[$domKey]) {
                    foreach (array_keys($gimp[$domKey]) as $key) {
...
function checkForMobile(){
    ...
    $isMobile = false;
    /* Comment by Tung Bui - 08/11/2017 - skip mobile layout
