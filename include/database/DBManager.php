...
protected function checkQuery($sql, $object_name = false)
{
    ...
    {
        //If there are no indices or table in this module then there is nothing to do.
        if (empty($indices) || empty($tableName)) {
            return '';
        }
        $take_action = false;
        ...
        foreach($compareIndices as $k => $value){
            ...
            $compareIndicesCI[strtolower($k)] = $value;
        }

        $compareIndices = $compareIndicesCI;
        unset($compareIndicesCI);
