...
class ViewQuickcreate extends ViewAjax
{
    ...
    public function display()
    {
        ...
        // Default templateMeta form values
        $form = array(
            'headerTpl' => $this->headerTpl,
            'footerTpl' => $this->footerTpl,
            'buttons' => $this->defaultButtons,
            'button_location' => 'bottom',
            'hideAudit' => true
        );
        // Override them in case a custom file is present
        $this->ev->defs['templateMeta']['form'] = array_merge(
            $form,
            empty($this->ev->defs['templateMeta']['form']) ? array() : $this->ev->defs['templateMeta']['form']
        );

        $form = array(
            'hidden' => array(
                '<input type="hidden" name="is_ajax_call" value="1" />',
                '<input type="hidden" name="from_dcmenu" value="1" />'
            ),
        );
        $this->ev->defs['templateMeta']['form'] = array_merge_recursive($this->ev->defs['templateMeta']['form'], $form);
