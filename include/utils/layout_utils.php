 ...
 */
function getClassicModuleTitle($moduleLabel, $params, $show_create=false, $index_url_override='', $create_url_override='')
{
	global $sugar_version, $sugar_flavor, $server_unique_key, $current_language, $action;
    ...
    $moduleLabel = preg_replace("/ /","",$moduleLabel);
    $iconPath = "";
    $the_title = "<div class='moduleTitle'><h2>\n";

    if(is_file(SugarThemeRegistry::current()->getImageURL('icon_'.$moduleLabel.'_32.png',false)))
    {
    	$iconPath = SugarThemeRegistry::current()->getImageURL('icon_'.$moduleLabel.'_32.png');
    } else if (is_file(SugarThemeRegistry::current()->getImageURL('icon_'.ucfirst($moduleLabel).'_32.png',false)))
    {
        $iconPath = SugarThemeRegistry::current()->getImageURL('icon_'.ucfirst($moduleLabel).'_32.png');
    }
    if (!empty($iconPath)) {
    	$url = (!empty($index_url_override)) ? $index_url_override : "index.php?module={$moduleLabel}&action=index";
        
        if($params[0] == $moduleLabel){
            $params[0] = "<a href='{$url}'><img style='margin-right:10px' src='{$iconPath}' ". "alt='".$moduleLabel."' title='".$moduleLabel."' align='absmiddle'>".$app_list_strings['moduleList'][$moduleLabel]."</a>";
        }
        else{
            array_unshift ($params,"<a href='{$url}'><img style='margin-right:10px' src='{$iconPath}' ". "alt='".$moduleLabel."' title='".$moduleLabel."' align='absmiddle'>".$app_list_strings['moduleList'][$moduleLabel]."</a>");    
        }  
	}
    
	$new_params = array_pop($params);
    if(!is_null($new_params) && ($new_params !== "")) $moduleLabel_title = $new_params;
    if(!in_array($module, array(
        'Home',
        'Administration',
        'Studio',
    ))){
        $params[] = '<span class="breadCrumbSymbol">»</span>';    
    }
    
    foreach($params as $value){
        $the_title .= $value;   
    }
    if(!empty($moduleLabel_title)){
        $the_title .= $moduleLabel_title."\n";//removing empty H2 tag for 508 compliance
    }
    ...
    if ($show_create) {
        ...
        {
            $create_url_override = "index.php?module={$moduleLabel}&action=EditView&return_module={$moduleLabel}&return_action=DetailView";
        }
...
EOHTML;
    ...
    $the_title .= "<div class='clear'></div></h2></div>\n";
