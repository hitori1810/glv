    ...
    class ListViewDisplay {
        ...
        */
        protected function buildActionsLink($id = 'actions_link', $location = 'top')
        {
            ...
            // compose sms - By HAI DUC
            $menuItems[] = $this->builsmsMenu($location);
...
EOF;
        ...
        */
        ...
        //Custom SMS - HAI DUC
        function builsmsMenu($location = 'top'){
			global $app_strings;
            // compose sms message
            ...
            if (isset($_REQUEST['module'])) {
                ...
                if (in_array($object, $e->get_modules_with_sms())) {
                    ...
                    $onclick = "if (sugarListView.get_checks_count()>0) openAjaxPopupForMulti('mass[]','{$_REQUEST['module']}',''); " .
                    "else alert('Please select at least 1 recipient'); return false;";
                    $menuItems= "<a href='javascript:void(0)'  onclick=\"{$onclick}\">".$app_strings['LBL_COMPOSE_SMS']."</a>";
                }
            }
            return  $menuItems;
        }
