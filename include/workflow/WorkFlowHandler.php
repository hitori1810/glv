...
class WorkFlowHandler {
    ...
    function WorkFlowHandler(&$focus, $event){
        ...
        //rrs Custom workflow after save By Lap Nguyen
        $after_save_list = $GLOBALS['sugar_config']['workflowAfterSave'];
        if( ($event=="before_save" && !in_array($focus->module_dir, $after_save_list)) || (!empty($after_save_list) && $event=="after_save" && in_array($focus->module_dir, $after_save_list))) { //Custom WF After Save By Lap Nguyen
