...
class PopupSmarty extends ListViewSmarty{
    ...
    */
    function _get_where_clause()
    {
        ...
        if($this->seed->module_dir  == 'C_Packages' ){
            if(!empty($where))$where .= ' AND ';
            $today = $GLOBALS['timedate']->nowDbDate();
            $where .= "((c_packages.end_date >= '$today' AND c_packages.start_date <= '$today') OR c_packages.end_date IS NULL )";  
        }
        if(!empty($_REQUEST['class_id_j_feedback_advanced']) && isset($_REQUEST['class_id_j_feedback_advanced'])){
            if(!empty($where))$where .= ' AND ';
            $where .= "((contacts.id IN (SELECT j_class_contacts_1contacts_idb FROM j_class_contacts_1_c WHERE j_class_contacts_1j_class_ida = '{$_REQUEST['class_id_j_feedback_advanced']}' AND deleted = 0)))";    
        }
        return $where;
    }
