	...
	for(var i = 0; i < validate[formname].length; i++){
			if(validate[formname][i][nameIndex].indexOf(startsWith) == 0){
				if(typeof form[validate[formname][i][nameIndex]]  != 'undefined' && typeof form[validate[formname][i][nameIndex]].value != 'undefined'){
					...
					if ((isFieldHidden(field, validate[formname][i][typeIndex]) && (validate[formname][i][typeIndex] != 'teamset' &&  validate[formname][i][typeIndex] != 'datetimecombo')) || field.disabled)
					{
						continue;
					}
...
/*
		...
		showHelpTips: function(el,helpText,myPos,atPos,id) {
				...
				var pos = $(el).offset(),
                    ofWidth = $(el).width(),
                    elmId = id || 'helpTip' + Math.round(pos.left) + '_' + ofWidth,
