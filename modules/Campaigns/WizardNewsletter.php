<?php
    ...
    ********************************************************************************/
    ...
    if( (isset($_REQUEST['wizardtype'])  && $_REQUEST['wizardtype']==1)  ||  ($focus->campaign_type=='NewsLetter')){
        ...
        $ss->assign("CAMPAIGN_DIAGNOSTIC_LINK", diagnose());
        // Add By Hai Duc
        # tracy: sets the campaign type
    }elseif( (isset($_REQUEST['wizardtype'])  && $_REQUEST['wizardtype']==4)  || ($focus->campaign_type=='SMS') ){
        $campaign_type = 'sms'; 
    ...
    if($campaign_type == 'general'){
        ...
        foreach($OptionsArr as $key=>$val){
            if($val =='Newsletter' || $val =='Email' || $val =='SMS' || $val =='' ){
                //do not add
            }else{
        ...
        # tracy: to get rid of the dropdown and fixate the campaign type into SMS Add By Hai Duc
    }elseif($campaign_type == 'sms'){
        //Assign Email as type of campaign being created an disable the select widget
        $ss->assign("CAMPAIGN_TYPE_OPTIONS", $mod_strings['LBL_SMS']);
        $ss->assign("SHOULD_TYPE_BE_DISABLED", "input type='hidden' value='SMS'"); 
...
EOQ;
    ...
    if($campaign_type == 'general'){
        ...
        # tracy: creates the steps and nav_items specific for sms Add By Hai Duc   
    }elseif($campaign_type == 'sms'){
        $steps = create_sms_steps();
        $ss->assign('NAV_ITEMS',create_wiz_menu_items($steps,'sms',$mrkt_string,$summ_url));
        $ss->assign('HIDE_CONTINUE','submit');
        #####
    }else{
    ...
    function create_wiz_menu_items($steps,$type,$mrkt_string,$summ_url){
        ...
        if($type == 'newsletter'  ||  $type == 'email'){
            ...
            $lbl = $type == 'sms' ? $mod_strings['LBL_NAVIGATION_MENU_SEND_SMS'] : $mod_strings['LBL_NAVIGATION_MENU_SEND_EMAIL'];
            $nav_html .= "<tr><td scope='row' nowrap><div id='nav_step".($i+2)."'>".$lbl."</div></li>";
            $nav_html .= "<tr><td scope='row' nowrap><div id='nav_step".($i+3)."'>".$summ_url."</div></td></tr>";
        }else{
            $nav_html .= "<tr><td scope='row' nowrap><div id='nav_step".($i)."'>".$summ_url."</div></td></tr>";
        }
