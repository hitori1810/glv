<?php
    ...
    ********************************************************************************/
    ...
    if(isset($_REQUEST['record']) &&  !empty($_REQUEST['record'])) {
        ...
        # tracy: only make the tracker available for non-SMS campaigns
        if ($focus->campaign_type != "SMS")
            $ss->assign("TRACKERS_TBL", create_tracker_summary ($focus));
        # tracy: add a condition if type is also SMS
        if($focus->campaign_type =='NewsLetter' || $focus->campaign_type =='Email' || $focus->campaign_type == "SMS"){
        ...
        if($focus->campaign_type == 'NewsLetter'){
            ...
            $ss->assign("CAMPAIGN_DIAGNOSTIC_LINK", diagnose());
        }elseif($focus->campaign_type == 'SMS'){
            $ss->assign('NAV_ITEMS',create_wiz_menu_items('sms',$mrkt_string,$camp_url,$summ_url ));
            //$ss->assign("CAMPAIGN_DIAGNOSTIC_LINK", diagnose()); 
        }else{
    ...
    function create_marketing_summary  ($focus){
        ...
        if ($focus->campaign_type == "SMS") require("modules/Campaigns/language/sms.lang.php");
        ...
        if ($focus->campaign_type!='SMS') {
            $mrkt_tbl .= "<td width='15%' scope='col'><b>".$mod_strings['LBL_FROM_MAILBOX_NAME']."</b></td>";
        }
        $mrkt_tbl .= "<td width='15%' scope='col'><b>".$mod_strings['LBL_STATUS_TEXT']."</b></td><td scope='col' colspan=2>&nbsp;</td></tr>";
    ...
    function create_wiz_menu_items($type,$mrkt_string,$camp_url,$summ_url){
        ...
        if($type == 'newsletter'  ||  $type == 'email' || $type == 'sms'){
            $nav_html .= "<td scope='row' nowrap><div id='nav_step'".($i+1).">$mrkt_string</div></td></tr>";
            if ($type == 'sms') {
                $lbl =$mod_strings['LBL_NAVIGATION_MENU_SEND_SMS'];
            } else {
                $lbl =$mod_strings['LBL_NAVIGATION_MENU_SEND_EMAIL'];
            }
            $nav_html .= "<td scope='row' nowrap><div id='nav_step'".($i+2).">{$lbl}</div></td></tr>";
            $nav_html .= "<td scope='row' nowrap><div id='nav_step'".($i+3).">".$summ_url."</div></td></tr>";
        }else{
            $nav_html .= "<td scope='row' nowrap><div id='nav_step'".($i).">".$summ_url."</div></td></tr>";
        }
