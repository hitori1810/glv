...
class SugarAuthenticate{
	...
	function loginAuthenticate($username, $password, $fallback=false, $PARAMS = array ()){
        ...
        if (
            !empty($usr->id)
            && $res['lockoutexpiration'] > 0
            && $usr->getPreference('loginfailed') >= ($res['lockoutexpirationlogin'])
            && !($usr->portal_only)
        ) {
        ...
        SugarApplication::setCookie('login_user_name_remember', '', time()-42000, '/');
		$_SESSION['login_password'] = $password;
        SugarApplication::setCookie('login_password_remember', '', time()-42000, '/');
