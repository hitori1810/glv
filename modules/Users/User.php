...
EOQ;
   ...
   function bean_implements($interface) {
     ...
     */
    public function setNewPassword($new_password, $system_generated = '0', $had_md5 = false)
    {
        if($had_md5)
            $user_hash = self::getPasswordMd5Hash($new_password);
        else
    ...
    function fill_in_additional_detail_fields() {
        ...
        //Get Team type - By Lap Nguyen
        //$this->team_type = $GLOBALS['db']->getOne("SELECT team_type FROM teams WHERE id='{$this->team_id}' AND deleted = 0");
        
        //Update by Tung Bui - Always user Junior type
        $this->team_type = 'Junior';
   ...
   {    //call parent method, specifying for array to be returned
    ...
    public static function generatePassword()
    {
        ...
        $SPECIAL = '~!@#$%^*()_+=-{}|';
        $condition = 0;
        $charBKT .= $UPPERCASE . $LOWERCASE . $NUMBER. $SPECIAL;
        $password = "";
    ...
    public function sendEmailForPassword($templateId, array $additionalData = array())
    {
        ...
        $htmlBody = str_replace('$contact_user_full_user_name', $this->full_user_name, $htmlBody);//Edit By Lam Hai 29/7/2016
        ...
        $body = str_replace('$contact_user_full_user_name', $this->full_user_name, $body);//Edit By Lam Hai 29/7/2016
        ...
        if ($result['status'] == true)
        {
            ...
            {
                $auto = isset($additionalData['system_generated_password']) ? $additionalData['system_generated_password'] : '1';
                $this->setNewPassword($additionalData['password'], $auto);
            }
        }
    ...
    public static function getPasswordMd5Hash($password) {
        if(!defined('CRYPT_MD5') || !constant('CRYPT_MD5')) {                  
            if(defined('CRYPT_EXT_DES') && constant('CRYPT_EXT_DES')) {
                return crypt(strtolower($password),
                    "_.012".substr(str_shuffle('./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'), -4));
            }                   
