...
class UserViewHelper {
    ...
    public function setupUserTypeDropdown() {
        ...
        $userTypes = array(
            ...
            // Add new type: TEMPLATE - Thuan Nguyen
            'TEMPLATE' => array(
                'label' => translate('LBL_TEMPLATE','Users'),
                'description' => translate('LBL_TEMPLATE_DESC','Users'),
            ),
            //END: Add new type TEMPLATE
            'Administrator' => array(
        ...
        // Add new type: TEMPLATE - Thuan Nguyen
        if ( $userType == 'GROUP' || $userType == 'PORTAL_ONLY' || $userType == 'TEMPLATE') { //END: Add new type TEMPLATE
    ...
    protected function setupPasswordTab() {
        ...
        // Add new type: TEMPLATE - Thuan Nguyen
        if(((isset($enable_syst_generate_pwd) && !$enable_syst_generate_pwd && $this->usertype!='GROUP' && $this->usertype!='TEMPLATE') || $this->usertype =='PORTAL_ONLY') && empty($this->bean->id)) { //END: Add new type TEMPLATE
        ...
        // Add new type: TEMPLATE - Thuan Nguyen
        if(($current_user->id == $this->bean->id || $this->usertype=='PORTAL_ONLY' || $this->usertype=='TEMPLATE' || (($this->usertype=='REGULAR' || $this->usertype == 'Administrator' || (isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true' && $this->usertype!='GROUP')) && !$enable_syst_generate_pwd)) && !$this->bean->external_auth_only ) { //END: Add new type TEMPLATE
        ...
        if ( isset($configurator->config['passwordsetting'])
             ...
             && $this->usertype != 'GROUP' && $this->usertype != 'PORTAL_ONLY' && $this->usertype != 'TEMPLATE') { //END: Add new type TEMPLATE
            $this->ss->assign('REQUIRED_EMAIL_ADDRESS','1');
        } else {
            $this->ss->assign('REQUIRED_EMAIL_ADDRESS','0');
        }
        // Add new type: TEMPLATE - Thuan Nguyen
        if ($this->usertype=='TEMPLATE') {
            $this->ss->assign('HIDE_FOR_TEMPLATE', 'none');
        }
        //END: Add new type TEMPLATE
...
// Build a list of themes that support group modules
    ...
    protected function setupAdvancedTabUserSettings()
    {
        ...
        if($this->bean->receive_notifications) $this->ss->assign("RECEIVE_NOTIFICATIONS", "checked");
    ...
    protected function setupAdvancedTabNavSettings() {
        ...
        if($user_subpanel_tabs) {
            $this->ss->assign("SUBPANEL_TABS", 'checked');
        } else {
            $this->ss->assign("SUBPANEL_TABS", '');
        }
    ...
    protected function setupAdvancedTabPdfSettings() {
        ...
        if ( PDF_CLASS == 'TCPDF' ) {
			...
			$this->ss->assign('PDF_FONT_NAME_MAIN',get_select_options_with_id($fontlist, 'dejavusans'));
            $this->ss->assign('PDF_FONT_NAME_MAIN_DISPLAY',$fontlist['dejavusans']);
            $this->ss->assign('PDF_FONT_SIZE_MAIN',PDF_FONT_SIZE_MAIN);
			$this->ss->assign('PDF_FONT_NAME_DATA',get_select_options_with_id($fontlist, 'dejavusans'));
    ...
    protected function setupEmailSettings() {
        ...
        // Add new type: TEMPLATE - Thuan Nguyen
        if ($this->usertype == 'GROUP' || $this->usertype == 'PORTAL_ONLY' || $this->usertype == 'TEMPLATE') { //END: Add new type TEMPLATE
            global $dictionary;
    ...
    function setUserType($user)
    {
        ...
        // Add new type: TEMPLATE - Thuan Nguyen
        else if ($user->is_template)
        {
            $user->user_type = 'TEMPLATE';
        }
        //END: Add new type TEMPLATE
        else
        {
