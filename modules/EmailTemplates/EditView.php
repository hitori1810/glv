<?php
    ...
    ********************************************************************************/
    ...
    if($has_campaign || $inboundEmail) {
        ...
        if ($has_campaign) {
            ...
            if ($campaign->campaign_type == 'SMS') {
                $for_sms_campaign = true;
            }
        } elseif (!empty($focus->sms_only)) {
            $for_sms_campaign = true;
        }
    ...
    if((isset($focus->sms_only) && $focus->sms_only) || $for_sms_campaign) {
    ...
    if ($for_sms_campaign) {
        $xtpl->assign("SMSONLY_CHECKED","CHECKED DISABLED"); 
        $xtpl->assign("SMS_ONLY", ($for_sms_campaign ? "1" : "0"));
    } elseif (!empty($focus->sms_only)) {
