...
eoq;
    ...
    function findEmailFromBeanIds($beanIds, $beanType, $whereArr) {
		...
		if ($person->ACLAccess('list')) {
			...
			if ($beanType === 'accounts') {
                            // bug63436 we need name here as that's what we have in vardefs for name_format_map
                            // but we don't want to get rid of last_name as it might still be used somewhere
				$t = "SELECT {$table}.id, '' first_name, {$table}.name last_name, {$table}.name name, eabr.primary_address, ea.email_address, '{$module}' module ";
			} else {
				$t = "SELECT {$table}.id, {$table}.first_name, {$table}.last_name, {$table}.last_name name, eabr.primary_address, ea.email_address, '{$module}' module ";
			}
