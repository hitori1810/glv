    ...
    ********************************************************************************/
    ...
    class EmailMan extends SugarBean{
        ...
        */
        ...
        function sendSMS($save_emails=1,$testmode=false,$user_id) {
            ...
            $module = new $class(); // tracy: the module
            $module->retrieve($this->related_id); // tracy: person's id
            ...
            if (empty($this->current_emailtemplate) or $this->current_emailtemplate->id != $this->current_emailmarketing->template_id) {
                ...
                //  $sms_message = $this->current_emailtemplate->body;
            }
            ...
            //Load Template By Lap Nguyen
            require_once('modules/EmailTemplates/EmailTemplate.php');
            $template = new EmailTemplate();
            $template->retrieve($this->current_emailmarketing->template_id);

//            $oContact = new $module->module_name();
//            $oContact->retrieve($module->id);  //Contact ID

            //Parse Body HTML
            $template->body = $template->parse_template_bean($template->body,$module->module_dir, $module);



            $sms->pname = $module->name;
            $res = $sms->send_message($phone_number, $template->body,$sms->parent_type,$sms->parent_id,$user_id);
