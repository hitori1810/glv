...
function getRoleActions($role_id, $type='module'){
        ...
        // Modified by Hieu Nguyen on 2017-08-09 to apply disabled modules
        require('custom/include/DisabledModules.php');
        
        if(!empty($disabledModules)) {
            $query .= " WHERE acl_actions.deleted=0 AND acl_actions.category NOT IN ('". join("', '", $disabledModules) ."') ORDER BY acl_actions.category, acl_actions.name";
        }
        else {
