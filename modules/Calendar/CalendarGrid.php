        ...
        protected function get_time_column($start){
            ...
            if($this->cal->view == 'month'){
                if($this->startday == 0)
                    ...
                    $wf = 0;
                //Custom code show calendar follow only - By Lap Nguyen
                $_REQUEST['only'] ? $pram = "&only={$_REQUEST['only']}" : $pram = "";    
                $head_content = "<a href='".ajaxLink("index.php?module=Calendar&action=index&view=week&hour=0&day=".$GLOBALS['timedate']->fromTimestamp($start)->format('j')."&month=".$GLOBALS['timedate']->fromTimestamp($start)->format('n')."&year=".$GLOBALS['timedate']->fromTimestamp($start)->format('Y'))."$pram'>".$GLOBALS['timedate']->fromTimestamp($start + $wf*3600*24)->format('W')."</a>";
                //END - By Lap Nguyen
            }
        ...
        */
        protected function get_basic_row($start,$cols = 7,$suffix = ""){
            ...
            if($this->cal->view == 'month' || $this->cal->style == "basic"){
                ...
                //Custom code show calendar follow only - By Lap Nguyen
                $_REQUEST['only'] ? $pram = "&only={$_REQUEST['only']}" : $pram = "";
                $head_content = "<a href='".ajaxLink("index.php?module=Calendar&action=index&view=week&hour=0&day=".$GLOBALS['timedate']->fromTimestamp($start)->format('j')."&month=".$GLOBALS['timedate']->fromTimestamp($start)->format('n')."&year=".$GLOBALS['timedate']->fromTimestamp($start)->format('Y'))."$pram'>".$head_content."</a>";
                //END - By Lap Nguyen
            }
        ...
        protected function get_day_head($start,$day = 0,$force = false){
            ...
            if($force){
                ...
                if($this->today_ts == $start)
                    $headstyle = " today";
                //Custom code show calendar follow param only - By Lap Nguyen
                $_REQUEST['only'] ? $pram = "&only={$_REQUEST['only']}" : $pram = "";
                $str .= "<div class='col_head".$headstyle."'><a href='".ajaxLink("index.php?module=Calendar&action=index&view=day&hour=0&day=".$GLOBALS['timedate']->fromTimestamp($start)->format('j')."&month=".$GLOBALS['timedate']->fromTimestamp($start)->format('n')."&year=".$GLOBALS['timedate']->fromTimestamp($start)->format('Y'))."$pram'>".$this->weekdays[$day]." ".$GLOBALS['timedate']->fromTimestamp($start)->format('d')."</a></div>";
                //END - By Lap Nguyen
            }
            return $str;
        }
        ...
        */
        protected function display_year(){
            ...
            for($m = 0; $m < 12; $m++){
                ...
                //Custom code show calendar follow param only - By Lap Nguyen
                $_REQUEST['only'] ? $pram = "&only={$_REQUEST['only']}" : $pram = "";
                $str .= '<a class="yearCalBodyMonthLink" href="'.ajaxLink('index.php?module=Calendar&action=index&view=month&&hour=0&day=1&month='.($m+1).'&year='.$GLOBALS['timedate']->fromTimestamp($month_start_ts)->format('Y')).$pram.'">'.$GLOBALS['app_list_strings']['dom_cal_month_long'][$m+1].'</a>';
                //END: By Lap Nguyen
                ...
                while($curr_time_global < $month_end_ts){
                    ...
                    for($d = 0; $d < 7; $d++){
                        ...
                        if($curr_time < $month_start_ts || $curr_time >= $month_end_ts)
                            ...
                            //Custom code show calendar follow param only - By Lap Nguyen
                            $_REQUEST['only'] ? $pram = "&only={$_REQUEST['only']}" : $pram = "";
                        $monC = '<a href="'.ajaxLink('index.php?module=Calendar&action=index&view=day&hour=0&day='.$GLOBALS['timedate']->fromTimestamp($curr_time)->format('j').'&month='.$GLOBALS['timedate']->fromTimestamp($curr_time)->format('n').'&year='.$GLOBALS['timedate']->fromTimestamp($curr_time)->format('Y')) . $pram.'">'.$GLOBALS['timedate']->fromTimestamp($curr_time)->format('j').'</a>';
                        //End: By Lap NGuyen
