    ...
    class CalendarActivity {
        ...
        */
        function get_activities($user_id, $show_tasks, $view_start_time, $view_end_time, $view, $show_calls = true, $show_completed = true)
        {
            ...
            $show_meetings = true;
            $only = $_REQUEST['only'];
            if($only =='Consultant' || $only=='Testing' || $only=='Meeting' || $only=='Session'){
                $show_calls = false;
                $show_tasks = false;  
            }else{
                if($only =='Task'){
            ...
            if(ACLController::checkAccess('Meetings', 'list', $current_user->id == $user_id) && $show_meetings) {
                //END custom - By Lap Nguyen
                $meeting = new Meeting();
                //Custom Show Session - By Lap Nguyen
                if($current_user->id  == $user_id || $_REQUEST['only'] == 'Session') {
                    $meeting->disable_row_level_security = true;
                }
                if($_REQUEST['only'] == 'Session') {
                  $where = CalendarActivity::get_occurs_within_where_clause($meeting->table_name, '', $view_start_time, $view_end_time, 'date_start', $view);  
                }else{
