...
CAL.create_item = function (params) {
    ...
    //Custom by Lap Nguyen - Colorrizing Calendar
    if (item.module_name =="Meetings" && typeof(CAL.activity_colors[item.meeting_type]) != 'undefined') { 
        el.style.backgroundColor = CAL.activity_colors[item.meeting_type]['body'];
        el.style.borderColor     = CAL.activity_colors[item.meeting_type]['border'];
    } 
    else {
    ...
    //Custon Show Session - By Lap Nguyen
    if(item.meeting_type !='Session'){
        ...
        for (w = 0; w < portions_count; w++) {
            ...
            CAL.each(portion, function (i, v) {
                ...
                if (slot) {
                    ...
                    if (CAL.items_draggable && item.edit == 1 && item.meeting_type !='Session') {//END
...
CAL.add_item_to_grid = function (item) {
    ...
    if (slot = CAL.get("t_" + time_cell + suffix)) {
        ...
        if (item.edit == 1) {
            //Custom Show Session - By Lap Nguyen
            if (CAL.items_draggable && item.meeting_type !='Session')
                CAL.make_draggable(elm_id, "advanced");
            if (item.module_name != "Tasks" && CAL.items_resizable && item.meeting_type !='Session')//END
...
//Custome add time week - By Lap Nguyen
CAL.get_header_text = function (type, time_start, text, record) {
    var start_text = (CAL.view == 'month' || CAL.view == 'week' || CAL.view == 'day' || CAL.view == 'shared') ? ("<span class='start_time'>" + time_start + "</span> " + text) : text;
    return start_text;
}
//End Custom
...
CAL.goto_date_call = function () {
    ...
    //Custom Show Session - By Lap Nguyen
    if(typeof GetURLParameter('only') == 'undefined')
    window.location.href = "index.php?module=Calendar&view=" + CAL.view + "&day=" + date_arr[1] + "&month=" + date_arr[0] + "&year=" + date_arr[2];
    else
    window.location.href = "index.php?module=Calendar&view=" + CAL.view + "&day=" + date_arr[1] + "&month=" + date_arr[0] + "&year=" + date_arr[2] + "&only=" + GetURLParameter('only'); 
    //END
}
