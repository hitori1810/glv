    ...
    class CalendarDisplay {
        ...
        */
        public $activity_colors = array(
            ...
            //Add Color for Meeting Type - Lap Nguyen
            'Session' => array(
                'border' => '#1C5FBD',
                'body'   => '#D2E5FC',
            ),
        ...
        */
        protected function get_next_calendar(){
            ...
            if($_REQUEST['module'] == "Calendar"){
                ...
                $str .= "<a href='".ajaxLink("index.php?action=index&module=Calendar&view=".$this->cal->view."&".$this->cal->get_neighbor_date_str("next"))."$pram'>";
              //END - By Lap Nguyen
            }else{
        ...
        */
        protected function get_previous_calendar(){
            ...
            $str = "";
            if($_REQUEST['module'] == "Calendar"){
                //Custom code show calendar follow only - By Lap Nguye
                $_REQUEST['only'] ? $pram = "&only={$_REQUEST['only']}" : $pram = "";  
                $str .= "<a href='".ajaxLink("index.php?action=index&module=Calendar&view=".$this->cal->view."&".$this->cal->get_neighbor_date_str("previous")."")."$pram'>";
               //END
        ...
        public function display_calendar_header($controls = true){
            ...
            if($controls){
                ...
                foreach($tabs as $tab){
                    ...
                    $_REQUEST['only'] ? $pram = "&only={$_REQUEST['only']}" : $pram = ""; 
                    $tabs_params[$tab]['link'] = "window.location.href='".ajaxLink("index.php?module=Calendar&action=index&view=". $tab 
                    . $this->cal->date_time->get_date_str())."$pram'";
