...
if (isset($_REQUEST['run_query']) && ($_REQUEST['run_query'] == 1)) {
    ...
    if (isset($_REQUEST['save_and_run_query']) && ($_REQUEST['save_and_run_query'] == 'on')) {
        ...
        $sugar_smarty->assign("custom_url", $saved_report_seed->custom_url);
        $sugar_smarty->assign("is_admin_data", $saved_report_seed->is_admin_data);
        $sugar_smarty->assign("row_number", $saved_report_seed->row_number);
        $is_admin_data_help = smarty_function_sugar_help(array("text"=>$mod_strings['LBL_IS_ADMIN_DATA_HELP']),$sugar_smarty);
        $sugar_smarty->assign("is_admin_data_help", $is_admin_data_help);
       if($saved_report_seed->is_admin_data == '1')
            $checked_is_admin_data = 'checked';
        else $checked_is_admin_data = '';
        if($saved_report_seed->row_number == '1')
            $checked_is_row_number = 'checked';
        else $checked_is_row_number = '';
        $sugar_smarty->assign("checked_is_admin_data", $checked_is_admin_data);
        $sugar_smarty->assign("checked_is_row_number", $checked_is_row_number);
        $sugar_smarty->assign("list_of", 	explode(",",$saved_report_seed->list_of));
        $sugar_smarty->assign("list_of_arr", get_select_options_with_id($app_list_strings['report_list_list'], (empty($saved_report_seed->list_of) ? '' : explode(",",$saved_report_seed->list_of))));
        //END

        //Limit Result By Hai Nguyen
        $sugar_smarty->assign("limit_result", $saved_report_seed->limit_result);
        //END
    ...
    $sugar_smarty->assign("report_type", $saved_report_seed->report_type);
    $custom_url_help = smarty_function_sugar_help(array("text"=>$mod_strings['LBL_CUSTOM_URL_HELP']),$sugar_smarty);
    $sugar_smarty->assign("custom_url_help", $custom_url_help);
    //END
