...
function get_system_tabs(){
    ...
    // Added by Tung Bui on 2018-06-29 to apply disabled modules
    require('custom/include/DisabledModules.php');
    
    if(!empty($disabledModules)) {
        foreach($disabledModules as $moduleName) {
            unset($system_tabs_result[$moduleName]);      
        }
    }
    // End Tung Bui
...
function get_tabs_system(){
    ...
    // Added by Hieu Nguyen on 2017-08-09 to apply disabled modules
    require('custom/include/DisabledModules.php');
    
    if(!empty($disabledModules)) {
        foreach($disabledModules as $moduleName) {
            unset($tabs[$moduleName]);
            unset($unsetTabs[$moduleName]);
        }
    }
    // End Hieu Nguyen
