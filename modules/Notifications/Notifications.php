...
class Notifications extends Basic
{
    ...
    public function retrieveUnreadCountFromDateEnteredFilter($date_entered)
    {
        ...
        global $current_user;
        $query = "SELECT count(id) as cnt FROM {$this->table_name} where is_read='0' AND deleted='0' AND assigned_user_id='{$current_user->id}' AND
    ...
    public function getUnreadNotificationCountForUser($user = null)
    {
        ...
        $query = "SELECT count(id) as cnt FROM notifications where is_read='0' AND deleted='0' AND assigned_user_id='{$user->id}' ";
