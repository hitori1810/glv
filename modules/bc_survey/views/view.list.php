...
class bc_surveyViewList extends ViewList {
    ...
    function display() {
        ...
        if (!$checkSurveySubscription['success']) {
            ...
            if ($_GET['view'] == 'poll') {
                $this->where .= "bc_survey.survey_type ='poll'";
            } else {
                $this->where .= "bc_survey.survey_type ='survey'";
            }
            parent::display();
            if ($_GET['view'] == 'poll') {
                $Search_title = <<<EOQ
            <script language = "javascript">
             $('.moduleTitle').find('h2').html('Search Poll');
            </script>
EOQ;
                echo $Search_title;
            }else{
                $Search_title = <<<EOQ
            <script language = "javascript">
             $('.moduleTitle').find('h2').html('Search Survey');
            </script>
EOQ;
                echo $Search_title;
        }
