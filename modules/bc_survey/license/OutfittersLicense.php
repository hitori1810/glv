...
if (!class_exists('OutfittersLicense')) {
    ...
    class OutfittersLicense {
        ...
        public static function validate() {
            ...
            $administration->retrieveSettings('SurveyPlugin');

            $currDate = date("Y/m/d");
            if ($validated['success'] == 1) {
                $administration->saveSetting("SurveyPlugin", "LastValidation", 1);
                $administration->saveSetting("SurveyPlugin", "LastValidationDate", $currDate);
                $administration->saveSetting("SurveyPlugin", "LastValidationMsg", '');
            } else {
                $administration->saveSetting("SurveyPlugin", "LastValidation", 0);
                $administration->saveSetting("SurveyPlugin", "LastValidationDate", $currDate);
                $administration->saveSetting("SurveyPlugin", "LastValidationMsg", !empty($validated['result']) ? $validated['result'] : 'There seems some error while validating your license for Customer Survey module. Please try again later.');
            }
