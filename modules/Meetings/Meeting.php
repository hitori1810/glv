...
class Meeting extends SugarBean {
    ...
    */
    ...
    function save($check_notify = FALSE) {
        ...
        //        if (!empty($this->type) && $this->type != 'Sugar' && $this->type != 'Practice' && $this->type != 'Skill' && $this->type != 'Connect Club' ) {
    ...
    function get_list_view_data() {
        ...
        if($date_db	< $today	) {
            $meeting_fields['TIME_START_END']= "<font class='overdueTask' style='white-space: nowrap;'>".substr($meeting_fields['DATE_START'],0, 10).': '.substr($meeting_fields['DATE_START'], 11,5).' - '.substr($meeting_fields['DATE_END'], 11)."</font>";
            $meeting_fields['DATE_START']= "<font class='overdueTask'>".$meeting_fields['DATE_START']."</font>";
        }else if($date_db	< $nextday) {
            $meeting_fields['TIME_START_END'] = "<font class='todaysTask' style='white-space: nowrap;'>".substr($meeting_fields['DATE_START'],0, 10).': '.substr($meeting_fields['DATE_START'], 11,5).' - '.substr($meeting_fields['DATE_END'], 11)."</font>";
            $meeting_fields['DATE_START'] = "<font class='todaysTask'>".$meeting_fields['DATE_START']."</font>";
        } else {
            $meeting_fields['TIME_START_END'] = "<font class='futureTask' style='white-space: nowrap;'>".substr($meeting_fields['DATE_START'],0, 10).': '.substr($meeting_fields['DATE_START'], 11,5).' - '.substr($meeting_fields['DATE_END'], 11)."</font>";
    ...
    */
    ...
    /**
    * function check this session cancel
    *
    * @author Trung Nguyen at 2015.11.26
    */
    function canCancel() {
        if ($this->can_cancel !== null) return $this->can_cancel;

        $session_status = $GLOBALS['db']->getOne("SELECT session_status FROM {$this->table_name} WHERE id = '{$this->id}'");
        ...
        global $timedate;
        $date_start   = strtotime($timedate->to_db($this->date_start));
        $date_end     = strtotime($timedate->to_db($this->date_end));
