...
(function( $ ){
    ...
    // Move settings here by Hieu Nguyen to access the setting globally
    var settings = {
        'top'  : true,
        'left' : false,
        'marginTop' : '0px',
    ...
    var methods = {
        ...
        init : function( options ) {
            ...
            return this.each(function(){
                ...
                // Set number of rows for the below logics. Added by Hieu Nguyen to fix bug on SugarCRM 6.5
                $this.attr('rows', $this.find('tbody').find('tr').length);
                ...
                if ( ! data ) {
                    ...
                    if (settings.top) {
                        topHeader = $this.clone(false)
                            ...
                            //.wrap("<div").parent()    Replaced be Hieu Nguyen to fix bug conflict with SugarCRM 6.5 theme
                            .wrap("<div class='topFreezeHeader' for='"+ $this.attr('id') +"'>").parent()
                            .css({ position: 'absolute', top: $this.offset().top, left: $this.offset().left });
                    }
        ...
        scroll : function( event ) {
            ...
            if (($.browser.msie && $.browser.version < 7) || navigator.platform.indexOf("iPad") != -1) {
                ...
                if (topHeader) {
                    topHeader.css(
                        //(scrollTop < tableTop) ?  Replaced by Hieu Nguyen to fix bug header always display even if the table is hidden
                        (scrollTop < tableTop || !table.is(':visible')) ?
                            { visibility: 'hidden' } :
                            { visibility: 'visible',
                              position: 'fixed',
                              width: table.css('width'),    // Added by Hieu Nguyen to set the header width equals to table width on the fly
                              top: settings.marginTop,  // Added by Hieu Nguyen to fix bug confict with SugarCRM 6.5 theme
                              left: tableLeft - scrollLeft,
                              'z-index': settings.layerLevel});
                }
