...
var $useForSubpanel = true;
    ...
    function preDisplay() {  
        // Check license when creating new user
        if(isset($_POST['isDuplicate']) || empty($this->bean->id)) {
            checkUserLimit(true);
        }
        /**
        * Reset reference to create new user from template
        * 
        * @author   Thuan Nguyen
        */
        if (isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true' && isset($_REQUEST['SaveAsUser'])) {
            $this->bean->is_group = 0;
            $this->bean->portal_only = 0;
            $this->bean->is_template = 0;
            $this->bean->last_name = '';
        }
        // END: Reset reference to create new user from template
        ...
        /**
        * Assign user template ID to edit view for save user template reference
        * 
        * @author   Thuan Nguyen
        */
        if (!isset($_REQUEST['user_template_id']) || $_REQUEST['user_template_id'] == '') {
            $_REQUEST['user_template_id'] = '';
        }
        $this->ss->assign('USER_TEMPLATE_ID',$_REQUEST['user_template_id']);
        //END: Assign user template ID to edit view for save user template reference
    }
    ...
    public function getMetaDataFile() {
        ...
        if($this->fieldHelper->usertype == 'TEMPLATE'){
            $userType = 'Template';
        }
    ...
    function display() {
        ...
        if ( isset($this->fieldHelper->usertype) && ($this->fieldHelper->usertype == 'GROUP'
             ...
             // Add new type: TEMPLATE - Thuan Nguyen
             || $this->fieldHelper->usertype == 'TEMPLATE'
             // END: Add new type TEMPLATE
            )) {
