<?php
    ...
    ********************************************************************************/
    ...
    function additionalDetailsMeeting($fields) {
        ...
        if($fields['MEETING_TYPE'] !='Session'){
            if(!empty($fields['STATUS'])) {
                $overlib_string .= '<b>'. $mod_strings['LBL_STATUS'] . '</b> ' . $fields['STATUS'];
                $overlib_string .= '<br>';
            }    
        }else{
            //Add field to Addional Detail - By Lap Nguyen
            if(!empty($fields['CLASS_ID'])){
                $overlib_string .= '<b>'. $mod_strings['LBL_CLASS_NAME'] . ':</b> ' . $fields['CLASS_NAME'];
                $overlib_string .= '<br>';  
            }
            if(!empty($fields['TEACHER_ID'])){
                $overlib_string .= '<b>'. $mod_strings['LBL_TEACHER_NAME'] . ':</b> ' . $fields['TEACHER_NAME'];
                $overlib_string .= '<br>';  
            }
            if(!empty($fields['ROOM_ID'])){
                $overlib_string .= '<b>'. $mod_strings['LBL_ROOM_NAME'] . ':</b> ' . $fields['ROOM_NAME'];
                $overlib_string .= '<br>';  
            }  
        }
