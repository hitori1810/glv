...
if((!isset($_REQUEST['isProfile']) && empty($_REQUEST['id'])) || empty($_REQUEST['type']) || !isset($_SESSION['authenticated_user_id'])) {
    ...
    //1.Image avatar in DCMenu: Load image default in case file image not exists in file upload - Fix by MTN - 25/04/2015
    if(isset($_REQUEST['isTempFile']) && ($_REQUEST['type']=="SugarFieldImage") && (isset($_REQUEST['isProfile'])) && !empty($_REQUEST['id'])) {
    ...
    //2.Image avatar in Activity Stream: Load image default in case file image not exists in file upload - Fix by MTN - 25/04/2015
    if(isset($_REQUEST['isTempFile']) && ($_REQUEST['type']=="SugarFieldImage") && (!isset($_REQUEST['isProfile'])) && !empty($_REQUEST['id'])) {
        if(!file_exists($local_location)){
            $local_location = "include/images/default-profile.png"; 
        }
    ...
    if(!file_exists( $local_location ) || strpos($local_location, "..")) {
        ...
        //Edit by Tung Bui - customize to export Excel file form Sugar's csv file
        if($_REQUEST['id'] == "ImportErrors"){
            $csvFile = fopen($download_location,"r");
            $contentArray = array();
            $isHeader = true;
            if($_REQUEST['tempName'] == "error_1.csv") $isHeader = false;    
            while(! feof($csvFile)){
                if($isHeader){
                    $isHeader = false;  
            ...
            fclose($csvFile);
            include_once("custom/modules/Import/ImportHelpers.php");
            exportErrorFile($_REQUEST['tempName'],$contentArray,$_REQUEST['import_module']); 
        }
        else{
