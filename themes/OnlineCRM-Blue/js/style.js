...
$(document).ready(function() {
    if (typeof sugar_theme_gm_current !== 'undefined') {
        sugar_theme_gm_current = sugar_theme_gm_current.replace(' ', '_')
    };
    SUGAR.themes.resizeSearch();
...
SUGAR.append(SUGAR.themes, {
    ...
    resizeSearch: function() {
        if (SUGAR.themes.enableResizeSearch) {
            ...
            var searchWidth = $("#dcmenu").width() - 200 - $("#quickCreate").width() - $("#globalLinksModule").width() - $("#dcmenuSugarCube").width() - $("#moduleList").attr("width");
            searchWidth *= .75;
            ...
            $('#sugar_spot_search_div').find("section ul").width(searchWidth - 120);
        }
    },
    resizeMenu: function() {
        ...
        var maxMenuWidth = $("#dcmenu").width() - 100 - $("#quickCreate").width() - $("#globalLinksModule").width() - $("#dcmenuSugarCube").width() - $("#dcmenuSearchDiv").width();
        ...
        if (menuItemsWidth > maxMenuWidth) {
            ...
            while (menuItemsWidth >= maxMenuWidth && menuLength-- > 3) {
            ...
            while (menuItemsWidth <= maxMenuWidth && (menuLength <= max_tabs)) {
                ...
                if ((menuNode.attr("id") != undefined && menuNode.attr("id").match(/moduleMenuOverFlow[a-zA-Z]*/)) || (menuItemsWidth + parseInt(menuNodeWidth)) > maxMenuWidth) {
                    break;
                }
                menuLength++;
                menuNode.find(".megamenu").css({
    ...
    globalToolTips: function() {
        ...
        $("#quickCreateUL span").tipTip({
            maxWidth: "auto",
            edgeOffset: 10,
            content: SUGAR.language.translate('', 'LBL_QUICK_CREATE_TITLE')
        });
        if (typeof($("#dcmenuSugarCube").attr("title")) != 'undefined') {
            $("#dcmenuSugarCube").tipTip({
                maxWidth: "auto",
                edgeOffset: 10
            });
        }
        $("#sugar_spot_search").tipTip({
            maxWidth: "auto",
            edgeOffset: 10
        });
        $("#glblSearchBtn").tipTip({
            maxWidth: "auto",
            edgeOffset: 10
        });
        $("#partner").children("a").each(function(index) {
            $(this).tipTip({
                maxWidth: "auto",
                edgeOffset: 10
            });
    ...
    sugar_theme_gm_switch: function(groupName, groupId) {
        ...
        SUGAR.themes.current_theme = sugar_theme_gm_current = groupId;
        $("#dcmenu").animate({
            top: '-=' + dcheight
        }, 200, function() {
            current_menu.hide();
...
YAHOO.util.Event.onContentReady("tabListContainerTable", function() {
    YUI({
        combine: true,
        timeout: 10000,
        base: "include/javascript/yui3/build/",
        comboBase: "index.php?entryPoint=getYUIComboFile&"
    }).use("anim", function(Y) {
        ...
        var onClick = function(e) {
            ...
            anim.set('to', {
                scroll: [y + node.get('scrollLeft'), 0]
            });
            anim.run();
        };
